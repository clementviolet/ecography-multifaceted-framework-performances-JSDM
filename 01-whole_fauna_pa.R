...
train_whole_fauna_pa <- train_whole_fauna %>%
  vegan::decostand(method = "pa") %>%
  as.matrix()

## Model design

model <- Hmsc(Y = train_whole_fauna_pa,
...
model_whole_fauna_pa <- sampleMcmc(model,
    samples = samples,
    thin = thin,
    transient = transient,
    nChains = nChains,
...
save(model_whole_fauna_pa, file = "Data/Model_Output/model_whole_fauna_pa.RData")
